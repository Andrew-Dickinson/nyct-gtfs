...
class TestFeedParseADivision(unittest.TestCase):
    ...
    def test_feed_header(self):
        ...
        self.assertEqual(
            "NYCT Subway Feed (gtfs), generated 2021-11-26 15:56:25, containing 285 trips", str(self.feed))
        ...
        self.assertEqual(datetime.fromisoformat(
            "2021-11-26T15:56:25"), self.feed.last_generated)
        self.assertEqual(["1", "2", "3", "4", "5", "6", "7", "S"], list(
            self.feed.trip_replacement_periods.keys()))
        for line, trip_replacement_period in self.feed.trip_replacement_periods.items():
            self.assertEqual(datetime.fromisoformat(
                "2021-11-26T16:26:25"), trip_replacement_period)
    ...
    def test_train_parse_underway(self):
        ...
        self.assertEqual(datetime.fromisoformat(
            "2021-11-26T15:03:00"), trip.departure_time)
        ...
        self.assertEqual(datetime.fromisoformat(
            "2021-11-26T15:56:17"), trip.last_position_update)
        ...
        self.assertEqual(
            '{"090300_1..N", IN_TRANSIT_TO 107N @15:56:17}', repr(trip))
        ...
        self.assertEqual(datetime.fromisoformat(
            '2021-11-26T15:57:47'), remaining_stops[0].arrival)
        self.assertEqual(datetime.fromisoformat(
            '2021-11-26T15:57:47'), remaining_stops[0].departure)
        self.assertEqual("{ID: 107N, Arr: 15:57:47, Dep: 15:57:47, Sched: T4, }", repr(
            remaining_stops[0]))
        ...
        self.assertEqual(datetime.fromisoformat(
            '2021-11-26T15:59:17'), remaining_stops[1].arrival)
        self.assertEqual(datetime.fromisoformat(
            '2021-11-26T15:59:17'), remaining_stops[1].departure)

        self.assertEqual('104N', remaining_stops[2].stop_id)
        self.assertEqual('231 St', remaining_stops[2].stop_name)
        self.assertEqual(datetime.fromisoformat(
            '2021-11-26T16:00:47'), remaining_stops[2].arrival)
        self.assertEqual(datetime.fromisoformat(
            '2021-11-26T16:00:47'), remaining_stops[2].departure)

        self.assertEqual('103N', remaining_stops[3].stop_id)
        self.assertEqual('238 St', remaining_stops[3].stop_name)
        self.assertEqual(datetime.fromisoformat(
            '2021-11-26T16:02:17'), remaining_stops[3].arrival)
        self.assertEqual(datetime.fromisoformat(
            '2021-11-26T16:02:17'), remaining_stops[3].departure)

        self.assertEqual('101N', remaining_stops[4].stop_id)
        self.assertEqual('Van Cortlandt Park-242 St',
                         remaining_stops[4].stop_name)
        self.assertEqual(datetime.fromisoformat(
            '2021-11-26T16:03:47'), remaining_stops[4].arrival)
    ...
    def test_train_parse_assigned(self):
        ...
        self.assertEqual(datetime.fromisoformat(
            "2021-11-26T15:56:30"), trip.departure_time)
        ...
        self.assertEqual(
            'Southbound 1 to South Ferry, departs origin 15:56:30 - train assigned', str(trip))
        ...
        self.assertEqual('Van Cortlandt Park-242 St',
                         trip.stop_time_updates[0].stop_name)
    ...
    def test_train_parse_unassigned(self):
        ...
        self.assertEqual(None, trip.current_stop_sequence_index)
        self.assertEqual(datetime.fromisoformat(
            "2021-11-26T16:06:30"), trip.departure_time)
        self.assertEqual("S", trip.direction)
        self.assertEqual(False, trip.has_delay_alert)
        self.assertEqual("South Ferry", trip.headsign_text)
        self.assertEqual(None, trip.last_position_update)
        ...
        self.assertEqual(
            'Southbound 1 to South Ferry, departs origin 16:06:30', str(trip))
    ...
    def test_stop_time_track_mismatch(self):
        ...
        self.assertEqual(
            "72 St: Scheduled to arrive on track 1. Actually arriving on track 2. ", str(stop_time))

        protobuf_stop_time.Extensions[nyct_subway_pb2.nyct_stop_time_update].ClearField(
            'actual_track')
        self.assertEqual("1", stop_time.scheduled_track)
        self.assertEqual(None, stop_time.actual_track)
        self.assertEqual(False, stop_time.unexpected_track_arrival)
        ...
        with open('test_data/a_division.nyct.gtfsrt', 'rb') as f:
            with open('../nyct_gtfs/gtfs_static/stops.txt', 'r') as stops:
                with open('../nyct_gtfs/gtfs_static/trips.txt', 'r') as trips:
                    self.feed = NYCTFeed(
                        '1', fetch_immediately=False, stops_txt=stops, trips_txt=trips)
                    self.feed.load_gtfs_bytes(f.read())
...
class TestFeedConstructor(unittest.TestCase):
    def test_feed_constructor_bogus_feed(self):
        self.assertRaises(ValueError, NYCTFeed, "alskfjdk",
                          fetch_immediately=False)

    def test_feed_constructor_ace(self):
        ace_feed = NYCTFeed("A", fetch_immediately=False)
        self.assertEqual(
            "https://api-endpoint.mta.info/Dataservice/mtagtfsfeeds/nyct%2Fgtfs-ace", ace_feed._feed_url)
    ...
    def test_refresh_uses_timeout(self):
        with open('test_data/a_division.nyct.gtfsrt', 'rb') as f:
            data = f.read()
        ...
        with patch('nyct_gtfs.feed.requests.get', return_value=mock_response) as mock_get:
            feed = NYCTFeed('1', fetch_immediately=False, timeout=3.14)
            feed.refresh()

            mock_get.assert_called_once_with(feed._feed_url, timeout=3.14)

    def test_refresh_async_uses_timeout(self):
        with open('test_data/a_division.nyct.gtfsrt', 'rb') as f:
            data = f.read()
        ...
        with patch('nyct_gtfs.feed.httpx.AsyncClient', return_value=mock_client):
            feed = NYCTFeed('1', fetch_immediately=False, timeout=7)
            asyncio.run(feed.refresh_async())

            mock_get.assert_awaited_once_with(feed._feed_url, timeout=7)
...
class TestFeedParseBDivision(unittest.TestCase):
    ...
    def test_feed_filtering(self):
        self.assertEqual(
            20,
            len(self.feed.filter_trips(line_id="A",
...
class TestFeedParseDelayAlerts(unittest.TestCase):
    ...
    def test_feed_filtering(self):
        ...
        delayed_trip = self.feed.filter_trips(
            line_id="2", has_delay_alert=True)[0]
        self.assertEqual(
            '{"120700_2..N01R", DELAY, IN_TRANSIT_TO 201N @21:42:41}', repr(delayed_trip))
...
class TestFeedFiltering(unittest.TestCase):
    ...
    def test_feed_filtering(self):
        ...
        self.assertEqual(17, len(self.feed.filter_trips(
            line_id="1", travel_direction="N")))
        self.assertEqual(12, len(self.feed.filter_trips(
            line_id="1", travel_direction="N", headed_for_stop_id="123N")))
        self.assertEqual(12, len(self.feed.filter_trips(
            line_id="1", headed_for_stop_id="123N")))
        self.assertEqual(12, len(self.feed.filter_trips(
            line_id="1", headed_for_stop_id=["123N"])))
        self.assertEqual(26, len(self.feed.filter_trips(
            line_id="1", headed_for_stop_id=["123N", "123S"])))
        self.assertRaises(TypeError, self.feed.filter_trips,
                          headed_for_stop_id=137123)
        self.assertEqual(
            9, len(self.feed.filter_trips(line_id="2", underway=False)))
        self.assertEqual(11, len(self.feed.filter_trips(
            line_id="3", underway=False, train_assigned=False)))
        self.assertEqual(2, len(self.feed.filter_trips(
            line_id="3", underway=False, train_assigned=True)))
        self.assertEqual(16, len(self.feed.filter_trips(shape_id="1..S03R")))
        self.assertEqual(16, len(self.feed.filter_trips(shape_id=["1..S03R"])))
        self.assertEqual(19, len(self.feed.filter_trips(
            shape_id=["1..S03R", "1..S04R"])))
        self.assertRaises(TypeError, self.feed.filter_trips, shape_id=137123)

        self.assertEqual(
            23,
            len(self.feed.filter_trips(line_id="4",
                updated_after=self.feed.last_generated - timedelta(minutes=5)))
        )
        ...
        self.assertEqual(
            ...
            len(
                self.feed.filter_trips(
                    line_id=["7", "7X"],
                    updated_after=self.feed.last_generated -
                    timedelta(minutes=5)
                )
            )
        )
